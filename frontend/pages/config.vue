  ...
  <div class="max-w-6xl mx-auto px-4 py-8">
    ...
    <template v-if="authenticated">
      ...
      <div class="flex flex-wrap gap-1 mb-6 border-b border-gray-200">
        <button
          ...
          :class="activeTab === tab.key ? 'border-primary-600 text-primary-700 font-medium' : 'border-transparent text-gray-500 hover:text-gray-800'"
          @click="switchTab(tab.key)"
        >{{ tab.label }}</button>
      </div>

      <div v-if="globalStatus" class="mb-4 px-4 py-2 rounded-lg text-sm" :class="globalStatusOk ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'">
        {{ globalStatus }}
      </div>
      ...
      <div v-if="activeTab === 'overview'">
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <p class="text-2xl font-bold text-primary-600">{{ overviewStats?.total_chunks ?? "—" }}</p>
            <p class="text-xs text-gray-400 mt-1">向量块总数</p>
          </div>
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <p class="text-2xl font-bold text-primary-600">{{ totalArticlesCount }}</p>
            <p class="text-xs text-gray-400 mt-1">已入库文章</p>
          </div>
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <p class="text-2xl font-bold text-primary-600">{{ overviewStats?.searches_today ?? "—" }}</p>
            <p class="text-xs text-gray-400 mt-1">今日搜索次数</p>
          </div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
          <div class="flex items-center justify-between mb-3">
            <h2 class="text-sm font-semibold">知识树概览</h2>
            <button class="text-xs text-primary-600 hover:underline" @click="loadTree">刷新</button>
          </div>
          <div v-if="treeLoading" class="text-center py-6 text-gray-400 text-sm">加载中…</div>
          <div v-else-if="treeData.length === 0" class="text-center py-6 text-gray-400 text-sm">暂无数据</div>
          <div v-else class="text-sm space-y-1 max-h-72 overflow-y-auto">
            <div v-for="row in overviewRows" :key="row.key" :style="{ paddingLeft: `${row.depth * 14}px` }" class="text-gray-600">
              <template v-if="row.isArticle">📄 {{ row.node.title || row.node.name }}</template>
              <template v-else><span class="font-medium text-gray-800">{{ nodeIcon(row.node.type) }} {{ row.node.name }}</span> <span class="text-xs text-gray-400">({{ row.count }})</span></template>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'tree'" class="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-4 items-start">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div class="flex items-center gap-1 p-2 border-b border-gray-200 flex-wrap">
            <button class="px-2 py-1 text-xs border border-gray-200 rounded disabled:opacity-40" :disabled="!selectedNode" @click="teRename">重命名</button>
            <button class="px-2 py-1 text-xs border border-gray-200 rounded disabled:opacity-40" :disabled="!selectedNode || selectedNode.type === 'article'" @click="teAddChild">新建子节点</button>
            <button class="px-2 py-1 text-xs border border-gray-200 rounded disabled:opacity-40" :disabled="!selectedNode" @click="teMove(-1)" title="上移（仅本地展示，不持久化）">↑</button>
            <button class="px-2 py-1 text-xs border border-gray-200 rounded disabled:opacity-40" :disabled="!selectedNode" @click="teMove(1)" title="下移（仅本地展示，不持久化）">↓</button>
            <button class="ml-auto px-2 py-1 text-xs border border-red-200 text-red-600 rounded disabled:opacity-40" :disabled="!selectedNode" @click="teDelete">删除</button>
          </div>
          <div class="max-h-[560px] overflow-y-auto">
            <div v-if="treeLoading" class="text-center py-8 text-gray-400 text-sm">加载中…</div>
            <div v-else-if="treeData.length === 0" class="text-center py-8 text-gray-400 text-sm">暂无数据</div>
            <template v-for="site in treeData" :key="site.name">
              <template v-for="row in rowsForNode(site)" :key="row.key">
                <div
                  ...
                  :class="selectedKey === row.key ? 'bg-primary-50 border-primary-500 text-primary-700' : 'border-transparent hover:bg-gray-50'"
                  :style="{ paddingLeft: `${8 + row.depth * 14}px` }"
                  @click="selectRow(row)"
                >
                  <svg v-if="!row.isArticle" class="w-3 h-3 text-gray-400 shrink-0 transition-transform" :class="{ 'rotate-90': expandedKeys.has(row.key) }" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" @click.stop="toggleExpanded(row.key)"><path stroke-linecap="round" stroke-linejoin="round" d="M9 18l6-6-6-6" /></svg>
                  <span v-else class="w-3 shrink-0"></span>
                  <span class="truncate">{{ row.isArticle ? '📄 ' + (row.node.title || row.node.name) : nodeIcon(row.node.type) + ' ' + row.node.name }}</span>
                  <span v-if="!row.isArticle" class="text-xs text-gray-400 shrink-0 ml-auto">{{ row.count }}项</span>
                </div>
              </template>
            </template>
          </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div class="p-3 border-b border-gray-200 flex items-center gap-2 flex-wrap">
            <span class="text-sm font-medium">{{ selectedNode ? `「${selectedNode.name || selectedNode.title}」的文章` : '从左侧选择节点以查看文章' }}</span>
            <span class="text-xs text-gray-400">{{ panelArticles.length ? panelArticles.length + ' 篇' : '' }}</span>
            <input v-if="panelArticles.length" v-model="panelSearch" placeholder="标题搜索…" class="ml-auto px-2 py-1 text-xs border border-gray-200 rounded" />
            <button v-if="panelChecked.size" class="px-2 py-1 text-xs border border-red-200 text-red-600 rounded" @click="batchDeleteChecked">批量删除（{{ panelChecked.size }}）</button>
          </div>
          <div v-if="!selectedNode" class="text-center py-16 text-gray-400 text-sm">请选择左侧节点</div>
          <div v-else-if="filteredPanelArticles.length === 0" class="text-center py-16 text-gray-400 text-sm">暂无文章</div>
          <div v-else class="overflow-x-auto">
            <table class="w-full text-sm">
              ...
              <tbody>
                <tr v-for="a in filteredPanelArticles" :key="a.article_id" class="border-b border-gray-50 hover:bg-gray-50">
                  <td class="p-2"><input type="checkbox" :checked="panelChecked.has(a.article_id)" @change="toggleChecked(a.article_id)" /></td>
                  <td class="p-2 max-w-[200px] truncate" :title="a.title">{{ a.title }}</td>
                  <td class="p-2 text-xs text-gray-500 max-w-[160px] truncate">{{ a.source_site || '—' }} / {{ a.chapter || '—' }}</td>
                  ...
                  <td class="p-2">
                    <div class="flex gap-2 items-center">
                      <a v-if="a.url" :href="a.url" target="_blank" class="text-xs text-primary-600 hover:underline">原文</a>
                      <button class="text-xs text-gray-500 hover:underline" @click="openEditModal(a)">编辑</button>
                      <button class="text-xs text-red-500 hover:underline" @click="singleDeleteArticle(a.article_id)">删除</button>
                    </div>
                  </td>
                </tr>
              </tbody>
      ...
      <!-- ══════════════ 录入单篇 ══════════════ -->
      <div v-if="activeTab === 'single'" class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 space-y-3 max-w-2xl">
        <div><label class="block text-xs font-medium text-gray-500 mb-1">文章标题 *</label><input v-model="singleForm.title" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" /></div>
        <div><label class="block text-xs font-medium text-gray-500 mb-1">原文 URL</label><input v-model="singleForm.url" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" /></div>
        <div class="grid grid-cols-2 gap-3">
          <div><label class="block text-xs font-medium text-gray-500 mb-1">来源站点</label><input v-model="singleForm.source_site" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" /></div>
          <div><label class="block text-xs font-medium text-gray-500 mb-1">顶层分类</label><input v-model="singleForm.category" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" /></div>
        </div>
        <div><label class="block text-xs font-medium text-gray-500 mb-1">章节路径（斜杠分隔）</label><input v-model="singleForm.chapter" placeholder="如：药物/HRT/雌激素药物" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" /></div>
        <div><label class="block text-xs font-medium text-gray-500 mb-1">标签（逗号分隔）</label><input v-model="singleForm.tags" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" /></div>
        <div>
          <label class="block text-xs font-medium text-gray-500 mb-1">内容标记</label>
          <div class="flex gap-2 flex-wrap">
            <label v-for="f in FLAGS" :key="f" class="flex items-center gap-1 text-xs px-2 py-1 border rounded-full cursor-pointer" :class="singleForm.flags.includes(f) ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-gray-200 text-gray-500'">
              <input type="checkbox" class="hidden" :checked="singleForm.flags.includes(f)" @change="toggleArrItem(singleForm.flags, f)" />{{ flagLabel(f) }}
            </label>
          </div>
        </div>
        <div><label class="block text-xs font-medium text-gray-500 mb-1">正文内容（Markdown）*</label><textarea v-model="singleForm.content" rows="10" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm font-mono"></textarea></div>
        <div class="flex items-center gap-3">
          ...
          <span v-if="singleResult" class="text-sm" :class="singleOk ? 'text-green-600' : 'text-red-600'">{{ singleResult }}</span>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'batch'" class="space-y-4 max-w-3xl">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
          <h2 class="text-sm font-semibold mb-3">默认元数据（文件无 frontmatter / 非文件夹上传时使用）</h2>
          <div class="grid grid-cols-2 gap-3 mb-3">
            <div><label class="block text-xs font-medium text-gray-500 mb-1">来源站点</label><input v-model="batchDefaults.source_site" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" /></div>
            <div><label class="block text-xs font-medium text-gray-500 mb-1">顶层分类</label><input v-model="batchDefaults.category" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" /></div>
          </div>
          <div>
            <label class="block text-xs font-medium text-gray-500 mb-1">默认内容标记</label>
            <div class="flex gap-2 flex-wrap">
              <label v-for="f in FLAGS" :key="f" class="flex items-center gap-1 text-xs px-2 py-1 border rounded-full cursor-pointer" :class="batchDefaults.flags.includes(f) ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-gray-200 text-gray-500'">
          ...
          <p class="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-md px-3 py-2 mt-3">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
          <div class="grid grid-cols-2 gap-3 mb-3">
            <label class="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center cursor-pointer hover:border-primary-400 hover:bg-primary-50/40 text-sm text-gray-500">
              选择 .md 文件（支持多选）
          ...
          <div v-if="batchFiles.length" class="space-y-1.5 mb-3 max-h-72 overflow-y-auto">
            <div v-for="f in batchFiles" :key="f.relPath" class="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-md px-3 py-1.5 text-xs">
              <span class="flex-1 truncate font-medium">{{ f.file.name }}</span>
          ...
          <div v-if="batchFiles.length" class="flex items-center gap-2">
            ...
            <button class="px-3 py-2 text-sm text-gray-500 hover:underline" @click="clearFiles">清空列表</button>
            <span class="text-xs text-gray-400">{{ batchProgress }}</span>
          </div>
      ...
      <div v-if="activeTab === 'cache'">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
          <div class="flex items-center justify-between mb-4">
            ...
            <button class="px-3 py-1 text-xs border border-gray-300 rounded-md text-gray-500 hover:bg-gray-50" @click="loadKeywords">刷新</button>
          </div>
          <div class="flex flex-wrap items-center gap-2 mb-4 p-3 bg-gray-50 rounded-lg">
            <input v-model="newKeyword" placeholder="关键词（如 HRT）" class="flex-1 min-w-[120px] px-3 py-1.5 border border-gray-300 rounded-md text-sm" />
            <input v-model="newExpansionsInput" placeholder="扩展词（逗号分隔）" class="flex-[2] min-w-[200px] px-3 py-1.5 border border-gray-300 rounded-md text-sm" />
            ...
            <button v-if="editingKeyword" class="px-3 py-1.5 border border-gray-300 rounded-md text-sm text-gray-500 hover:bg-gray-50" @click="cancelEdit">取消</button>
          </div>
          <div v-if="keywordsLoading" class="text-center py-4 text-gray-400 text-sm">加载中…</div>
          <div v-else-if="Object.keys(keywords).length === 0" class="text-center py-4 text-gray-400 text-sm">暂无高频词缓存，添加后搜索将优先使用预设扩展词</div>
          <div v-else class="space-y-2">
            <div v-for="(entry, key) in keywords" :key="key" class="flex items-start gap-2 p-3 border border-gray-100 rounded-lg hover:bg-gray-50">
              <div class="flex-1 min-w-0">
                <span class="font-mono text-sm font-medium text-gray-800">{{ key }}</span>
                <div class="text-xs text-gray-500 mt-1">↳ {{ entry.expansions.join("、") }}</div>
              </div>
              <div class="flex gap-1 shrink-0">
                <button class="px-2 py-1 text-xs border border-gray-200 rounded text-gray-500 hover:bg-gray-100" @click="editKeyword(key as string, entry.expansions)">编辑</button>
                <button class="px-2 py-1 text-xs border border-red-200 rounded text-red-500 hover:bg-red-50" @click="removeKeyword(key as string)">删除</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
          <div class="flex items-center justify-between">
            ...
            <button class="px-3 py-1 text-xs border border-red-200 rounded-md text-red-500 hover:bg-red-50" @click="clearCache">清除全部</button>
          </div>
        ...
        <div v-if="config" class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
          ...
          <div class="space-y-4">
            <div v-for="(val, key) in config" :key="key" class="flex items-center justify-between">
              ...
              <span v-if="typeof val === 'boolean'" class="text-sm"><span :class="val ? 'text-green-600' : 'text-gray-400'">{{ val ? "开启" : "关闭" }}</span></span>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
          ...
          <div class="space-y-3">
            <div v-for="field in editableFields" :key="field.key" class="flex items-center gap-3">
              ...
              <input v-if="field.type === 'text'" v-model="editForm[field.key]" class="flex-1 px-3 py-1.5 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-400" />
              <input v-else-if="field.type === 'number'" v-model.number="editForm[field.key]" type="number" class="flex-1 px-3 py-1.5 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-400 w-24" />
              <select v-else-if="field.type === 'boolean'" v-model="editForm[field.key]" class="px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white focus:outline-none focus:ring-1 focus:ring-primary-400">
    ...
    <div v-if="editModal.open" class="fixed inset-0 bg-black/45 flex items-center justify-center z-50 p-4" @click.self="editModal.open = false">
      <div class="bg-white rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <h3 class="text-lg font-bold mb-4">编辑文章</h3>
        <div class="space-y-3">
          <div><label class="block text-xs font-medium text-gray-500 mb-1">标题</label><input v-model="editModal.title" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" /></div>
          <div><label class="block text-xs font-medium text-gray-500 mb-1">原文 URL</label><input v-model="editModal.url" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" /></div>
          <div class="grid grid-cols-2 gap-3">
            <div><label class="block text-xs font-medium text-gray-500 mb-1">来源站点</label><input v-model="editModal.source_site" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" /></div>
            <div><label class="block text-xs font-medium text-gray-500 mb-1">分类</label><input v-model="editModal.category" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" /></div>
          </div>
          <div><label class="block text-xs font-medium text-gray-500 mb-1">章节路径</label><input v-model="editModal.chapter" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" /></div>
          <div><label class="block text-xs font-medium text-gray-500 mb-1">标签（逗号分隔）</label><input v-model="editModal.tags" class="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" /></div>
          <div>
            <label class="block text-xs font-medium text-gray-500 mb-1">内容标记</label>
            <div class="flex gap-2 flex-wrap">
              <label v-for="f in FLAGS" :key="f" class="flex items-center gap-1 text-xs px-2 py-1 border rounded-full cursor-pointer" :class="editModal.flags.includes(f) ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-gray-200 text-gray-500'">
        ...
        <div class="flex justify-end gap-2 mt-5">
          <button class="px-4 py-2 text-sm border border-gray-300 rounded-lg text-gray-600" @click="editModal.open = false">取消</button>
