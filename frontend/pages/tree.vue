    ...
    <div v-if="!loading && tree.length > 0" class="space-y-4">
      <div v-for="site in tree" :key="site.name" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <template v-for="row in rowsForSite(site)" :key="row.key">
          ...
          <button
            v-if="row.isArticle"
            class="w-full flex items-center gap-2 py-1.5 hover:bg-white text-left group border-b border-gray-50 last:border-0"
            :style="{ paddingLeft: `${14 + row.depth * 14}px`, paddingRight: '14px' }"
            :title="row.node.title || row.node.name"
            @click="searchArticle(row.node)"
          >
            <span
            ...
            ></span>
            <span class="text-sm text-gray-700 truncate group-hover:text-primary-600">{{ row.node.title || row.node.name }}</span>
            <span v-for="t in (row.node.tags || []).slice(0, 3)" :key="t" class="text-[10px] text-gray-400 bg-gray-100 px-1.5 rounded shrink-0">{{ t }}</span>
            <a
              v-if="row.node.url"
              :href="row.node.url"
              target="_blank"
          ...
          <div
            v-else
            class="w-full flex items-center gap-2 py-2 hover:bg-gray-50 cursor-pointer border-b border-gray-50 last:border-0"
          ...
          >
            ...
            <span class="text-sm truncate" :class="row.depth === 0 ? 'font-semibold text-gray-800' : 'text-gray-700'">{{ row.node.name }}</span>
            <span class="text-xs text-gray-400 shrink-0">{{ row.count }} 篇</span>
